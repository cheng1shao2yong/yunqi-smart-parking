...
use app\parking\service\ParkingAuthService;
...
class ParkingBase extends BaseController
{
    /**
     * 当前登录用户
     * @var \app\parking\service\ParkingAuthService
     */
    protected $auth;
    ...
    /**
     * 停车场
     * @var \app\common\model\manage\Parking
     */
    protected $parking;
    ...
    protected function _initialize()
    {
        ...
        $this->auth = ParkingAuthService::newInstance(compact('modulename','controllername','actionname'));
        ...
        if(!$noNeedLogin){
            if ($this->auth->isLogin()) {
                ...
                if($this->request->isAjax()){
                    $response = Response::create(__('登录已超时，请重新登录!'), 'html', 403);
                    throw new HttpResponseException($response);
                }else{
                    $url=url('/login')->build();
                    redirect($url)->send();
